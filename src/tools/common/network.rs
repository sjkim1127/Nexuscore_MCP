...
impl Tool for NetworkCapture {
    ...
    fn description(&self) -> &str {
        "Starts/Stops packet capture using Tshark. Args: action ('start'/'stop'), interface (string), output (string, optional)"
    }
            ...
            "start" => {
                ...
                match child {
                    Ok(c) => {
                        ...
                        Ok(
                            serde_json::json!({ "status": "capture_started", "pid": pid, "file": output }),
                        )
                    }
