...
mod contract_tests {
    ...
    async fn test_sr_02_success_response_structure() {
        ...
        let result = server
            .call_tool_internal("get_metrics".into(), json!({}))
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            false,
            "Success tool should have is_error: false"
        );
        assert!(
            result.structured_content.is_some(),
            "Success tool should have structured_content"
        );
        ...
        assert!(
            sc["metadata"]["duration_ms"].is_number(),
    ...
    async fn test_sr_01_error_mapping_unknown_tool() {
        let server = NexusCoreServer::new();
        let result = server
            .call_tool_internal("non_existent_tool".into(), json!({}))
            .await
            .unwrap();
        ...
        assert!(
            ...
            "Unknown tool should have is_error: true"
        );
    ...
    async fn test_error_propagation_from_tool() {
        ...
        assert!(
            result.is_error.unwrap_or(false),
            "Tool failure should have is_error: true"
        );
        ...
        let sc = result
            .structured_content
            .as_ref()
            .expect("Failed tool should have structured_content");
        assert_eq!(sc["status"], "error");
        assert!(sc["error"].is_string(), "Error message should be a string");
        assert!(
            sc["error"].as_str().unwrap().contains("Missing value"),
    ...
    async fn test_large_result_summary_structure() {
        ...
        assert!(
            res["error"].is_string(),
            "Should have a notice in the error field for LLM"
        );
    }
}
