...
impl Tool for SystemDiff {
    ...
    fn description(&self) -> &str {
        "Takes/compares filesystem snapshots. Args: action (take/compare), paths (array of dirs to scan, default [C:\\Users\\Public])"
    }
    ...
    async fn execute(&self, args: Value) -> Result<Value> {
        ...
        match action {
            "take" => {
                ...
                let files = tokio::task::spawn_blocking(move || scan_files_blocking(paths_clone))
                    .await
            ...
            "compare" => {
                ...
                let new_files =
                    tokio::task::spawn_blocking(move || scan_files_blocking(paths_clone))
                        .await
                        .map_err(|e| anyhow::anyhow!("Spawn error: {}", e))?;
            ...
            _ => Ok(StandardResponse::error(
                tool_name,
                &format!("Unknown action: {}. Use 'take' or 'compare'", action),
            )),
        }
    }
}
